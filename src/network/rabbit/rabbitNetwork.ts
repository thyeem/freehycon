...
export class RabbitNetwork implements INetwork {
    ...
    public async connect(host: string, port: number, save: boolean = false): Promise<RabbitPeer> {
        ...
        try {
            const peerPromise = new Promise<RabbitPeer>(async (resolve, reject) => {
                ...
                socket.connect({ host, port }, async () => {
                    try {
                        const newPeer = await this.newConnection(socket, false)
    ...
    private async newConnection(socket: net.Socket, save: boolean = false): Promise<RabbitPeer> {
